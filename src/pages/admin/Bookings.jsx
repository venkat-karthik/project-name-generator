    ...
    <AdminLayout>
      <div style={{ padding: '24px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px', marginBottom: '32px' }}>
          {[
            { label: 'Total Bookings', value: bookings.length, color: '#c9a84c' },
            { label: 'Pending', value: bookings.filter(b => b.status === 'pending').length, color: '#e4c677' },
            { label: 'Confirmed', value: bookings.filter(b => b.status === 'confirmed').length, color: '#4ade80' },
            { label: 'Completed', value: bookings.filter(b => b.status === 'completed').length, color: '#60a5fa' },
          ].map(stat => (
          ...
          {['all', 'pending', 'confirmed', 'completed', 'cancelled'].map(status => (
            <button
              ...
              style={{
                ...
                borderRadius: '8px',
                border: 'none',
                background: filter === status ? '#c9a84c' : '#1a1a1a',
                color: filter === status ? '#0a0a0a' : '#888',
                cursor: 'pointer',
                fontWeight: 500,
                fontSize: '13px',
                transition: 'all 0.2s',
                  ...
                  <div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '12px', marginBottom: '12px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '13px', color: '#888' }}>
                        <Mail size={14} />
                        <a href={`mailto:${booking.email}`} style={{ color: '#c9a84c', textDecoration: 'none' }}>{booking.email}</a>
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '13px', color: '#888' }}>
                        <Phone size={14} />
                        <a href={`tel:${booking.phone}`} style={{ color: '#c9a84c', textDecoration: 'none' }}>{booking.phone}</a>
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '13px', color: '#888' }}>
                        <Calendar size={14} />
